...
interface AnimateChangeInHeightProps { children: React.ReactNode; className?: string }
export const AnimateChangeInHeight: React.FC<AnimateChangeInHeightProps> = ({ children, className }) => {
  ...
  return (
    <motion.div className={cn(className, "overflow-hidden")} style={{ height }} animate={{ height }} transition={{ duration: 0.1, ease: "easeIn" }}>
...
const FilterOperatorDropdown = ({ filterType, operator, filterValues, setOperator }: {
  filterType: FilterType; operator: FilterOperator; filterValues: string[];
  setOperator: (op: FilterOperator) => void;
}) => {
  const ops = filterValues.length > 1
...
const FilterValueCombobox = ({ filterType, filterValues, setFilterValues }: {
  filterType: FilterType; filterValues: string[];
  setFilterValues: (v: string[]) => void;
}) => {
  const [open, setOpen] = useState(false);
  const [input, setInput] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);
  const nonSelected = filterViewToFilterOptions[filterType]?.filter((f) => !filterValues.includes(f.name));

  return (
    <Popover open={open} onOpenChange={(o) => { setOpen(o); if (!o) setTimeout(() => setInput(""), 200); }}>
      ...
      <PopoverContent className="w-[200px] p-0">
        <AnimateChangeInHeight>
          <Command>
            <CommandInput placeholder={filterType} className="h-9" value={input} onInputCapture={(e) => setInput(e.currentTarget.value)} ref={inputRef} />
            <CommandList>
              <CommandEmpty>Tidak ditemukan.</CommandEmpty>
              <CommandGroup>
                {filterValues.map((v) => (
                  <CommandItem key={v} className="group flex gap-2 items-center" onSelect={() => { setFilterValues(filterValues.filter((x) => x !== v)); setTimeout(() => setInput(""), 200); setOpen(false); }}>
              ...
              {nonSelected?.length > 0 && (
                <>
                  ...
                  <CommandGroup>
                    {nonSelected.map((f) => (
                      <CommandItem key={f.name} value={f.name} className="group flex gap-2 items-center"
                        onSelect={(val) => { setFilterValues([...filterValues, val]); setTimeout(() => setInput(""), 200); setOpen(false); }}>
...
export default function Filters({ filters, setFilters }: { filters: Filter[]; setFilters: Dispatch<SetStateAction<Filter[]>> }) {
  return (
    <div className="flex gap-2 flex-wrap">
      {filters.filter((f) => f.value?.length > 0).map((filter) => (
