      ...
      <div className="max-w-7xl mx-auto space-y-16">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <p className="text-slate-500 max-w-md md:text-right text-lg">
            Ikuti empat langkah utama ini untuk memastikan Anda diterima dan
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {STEPS.map((step, index) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-xl ${step.highlighted ? "bg-primary text-white shadow-md" : "bg-white text-primary border border-slate-200 shadow-sm"}`}
              >
                {step.number}
              </div>
