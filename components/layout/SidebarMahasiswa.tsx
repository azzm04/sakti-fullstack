...
import { LayoutDashboard, Bell, Bot, Send, LogOut, BookMarked, Award } from "lucide-react";
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center px-4 py-3.5 rounded-xl transition-all duration-200 overflow-hidden ${isActive
