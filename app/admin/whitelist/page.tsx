...
function SesiWAR() {
  ...
  async function handleDistribusi() {
    ...
    if (
      !confirm(
        `Distribusikan mahasiswa ke ${slots.length} pewawancara?\nTindakan ini tidak bisa dibatalkan.`,
      )
    )
      return;
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
            ...
            <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden mb-4">
              <motion.div
                ...
                initial={{ width: 0 }}
                animate={{
                  width: `${(slots.length / sesi.kuota_pewawancara) * 100}%`,
                }}
              ...
              {Array.from({ length: sesi.kuota_pewawancara }, (_, i) => {
                ...
                return (
                  <div
                    ...
                    title={
                      slot
                        ? `${slot.pewawancara?.nama ?? "—"} (${new Date(slot.claimed_at).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })})`
                        : `Slot ${i + 1} — kosong`
                    }
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1">
                    Jumlah slot WAR yang tersedia (default: 20)
                  </p>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">
