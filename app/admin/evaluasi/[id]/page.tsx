...
/**
 * Halaman detail evaluasi wawancara per mahasiswa.
 *
 * TODO (realtime): Ganti DUMMY_DATA dengan fetch ke /api/admin/evaluasi/[id]
 * yang mengambil data kandidat + hasil wawancara dari database.
 * Admin bisa update/delete kolom wawancara dari halaman ini.
 */
...
import { DUMMY_DATA, type MahasiswaEvaluasi } from "../page";
...
export default function EvaluasiDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // TODO: ganti dengan fetch realtime — const { data } = useSWR(`/api/admin/evaluasi/${id}`)
  const mahasiswa: MahasiswaEvaluasi | undefined = DUMMY_DATA.find((m) => m.id === id);

  const [form, setForm] = useState<EvaluasiForm>({
    prestasi:    mahasiswa?.prestasi    ?? "",
    rekomendasi: mahasiswa?.rekomendasi ?? "",
    alasan:      mahasiswa?.alasan      ?? "",
    pewawancara: mahasiswa?.pewawancara ?? "",
  });
    ...
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/admin/evaluasi" className="text-sm text-primary hover:underline">
            ← Kembali ke daftar
          </Link>
        </div>
      </div>
  ...
  async function handleSave() {
    ...
    try {
      // TODO: ganti dengan fetch realtime
      // await fetch(`/api/admin/evaluasi/${id}`, {
      //   method: "PATCH",
  ...
  async function handleDelete() {
    // TODO: await fetch(`/api/admin/evaluasi/${id}`, { method: "DELETE" })
    ...
    <div className="min-h-screen bg-surface p-6 md:p-10">
      ...
      <div className="mb-8">
        <nav className="flex items-center gap-1.5 mb-3 text-[11px] uppercase tracking-wider font-semibold">
          ...
          <span className="text-muted-foreground">›</span>
          <span className="text-primary truncate max-w-[200px]">{mahasiswa.nama}</span>
        </nav>
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1 font-mono">{mahasiswa.no_pendaftaran_kipk}</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <dl className="space-y-3">
              {[
                { icon: BookOpen, label: "Prodi",  value: mahasiswa.prodi },
                { icon: User,     label: "NIK",    value: mahasiswa.nik   },
                { icon: Phone,    label: "No. HP", value: mahasiswa.no_hp },
          ...
          >
            ...
            <p className={`text-[11px] mt-1 ${isComplete ? "text-emerald-600" : "text-amber-600"}`}>
              {isComplete
                ? "Semua kolom wawancara sudah terisi."
                : "Rekomendasi dan nama pewawancara wajib diisi."}
            </p>
          </motion.div>
        </div>

        {/* ── Kanan: Form evaluasi (editable) ── */}
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="bg-white rounded-2xl border border-border shadow-sm p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">Diisi oleh pewawancara — admin dapat mengedit</p>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Juara olimpiade, beasiswa, organisasi, dll."
                  className="w-full px-3 py-2.5 text-sm border border-border rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 bg-slate-50 transition-all"
                />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground mb-5">
              Data evaluasi wawancara <span className="font-semibold text-on-surface">{mahasiswa.nama}</span> akan dihapus. Tindakan ini tidak dapat dibatalkan.
            </p>
