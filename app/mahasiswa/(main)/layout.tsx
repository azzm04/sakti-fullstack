  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-body text-slate-600">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                {loading ? (
                  ...
                  <>
                    <p className="text-sm font-bold text-slate-700">{user?.nama ?? "-"}</p>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-full flex justify-around items-center px-4 py-3 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-50">
        <Link href="/mahasiswa/dashboard" className="flex flex-col items-center justify-center text-primary active:scale-95 transition-transform">
          <div className="bg-blue-50 px-5 py-1 rounded-full mb-1">
            <Home className="w-5 h-5" />
          </div>
          <span className="text-[10px] font-bold">Home</span>
        </Link>
        <Link href="#" className="flex flex-col items-center justify-center text-slate-400 hover:text-primary active:scale-95 transition-transform">
